...
===============
  ...
  *Report bugs.
  ...
  For any case:
  Send a mail to zoelabbb@gmail.com with the modifications and information:
Email, Autor and changes. Thanks!!!
...
public:
  ...
  ~Console() { printf("\033[m"); }

  void clearLine() { printf("\033[K"); }

  void insertLine() { printf("\x1b[1L"); }

  void deleteLine() { printf("\033[1M"); }

  void setCursorPosition(int x, int y) { printf("\033[%d;%df", y, x); }

  void clearScreen() { printf("\033[%dm\033[2J\033[1;1f", background_color); }

  void setBackgroundColor(int color) { background_color = (color % 16) + 40; }
  ...
  int ungetch(int ch) { return ungetc(ch, stdin); }
  ...
  int getch() { return setEchoMode(false); }

  int getche() { return setEchoMode(true); }
  ...
  int gettext(int l, int t, int r, int b, void *destination) {
    // Provide a meaningful implementation or remove the parameters if not
    // needed.
    return 0;
  }
